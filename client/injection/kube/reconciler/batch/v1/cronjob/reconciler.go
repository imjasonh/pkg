...
// controller reconciling v1.CronJob.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.CronJob. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.CronJob) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.CronJob.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.CronJob. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.CronJob) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.CronJob if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.CronJob.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.CronJob) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.CronJob) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.CronJob resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client kubernetes.Interface, lister batchv1.CronJobLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.CronJob, desired *v1.CronJob) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.BatchV1().CronJobs(desired.Namespace)
		...
		updater := r.Client.BatchV1().CronJobs(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.CronJob) (*v1.CronJob, error) {
	...
	patcher := r.Client.BatchV1().CronJobs(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.CronJob) (*v1.CronJob, error) {
